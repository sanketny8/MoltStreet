...
async def test_only_moderator_can_resolve(client: AsyncClient):
    ...
    moderator_response = await client.post("/agents", json={
        "name": "moderator-can-resolve",
        "role": "moderator"
    })
    ...
    response = await client.post(f"/markets/{market_id}/resolve", json={
        "moderator_id": trader_id,
        "outcome": "YES"
    })
...
async def test_resolve_already_resolved_market(client: AsyncClient):
    ...
    # First resolution
    await client.post(f"/markets/{market_id}/resolve", json={
        "moderator_id": moderator_id,
        "outcome": "YES"
    })
...
async def test_losers_get_nothing(client: AsyncClient):
    ...
    market_response = await client.post("/markets", json={
        ...
        "question": "Testing loser payouts",
        "deadline": deadline
    })
    market_id = market_response.json()["id"]
